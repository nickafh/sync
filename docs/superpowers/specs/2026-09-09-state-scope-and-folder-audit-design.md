...
- After the cleanup, kept rows whose `phone_list_id` is not attached to the tunnel are re-pointed to the canonical phone list in one `ExecuteUpdateAsync` (not in a dry run). The unique index `(source_user_id, phone_list_id, target_mailbox_id, tunnel_id)` cannot conflict: after the dedupe there is exactly one row per source user for the pair. Log Information: `Re-pointed {Count} state row(s) from retired phone list(s) to list {PhoneListId} for tunnel {TunnelId} in mailbox {MailboxId}`.
...
- Integration (Postgres): migration asserts both columns; the re-point update against the real unique index with a retired and a canonical row for one user.
